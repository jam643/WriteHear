function [Dist,D,rw,tw,k]=dtw(r,t,band,pflag)
%
...
d=abs(repmat(r',1,N)-repmat(t,M,1)); %this makes clear the above instruction Thanks Pau Micó

D=zeros(size(d));
D(:)=inf;
D(1,1)=d(1,1);

for m=2:min(M,ceil(band*M)+1)
    D(m,1)=d(m,1)+D(m-1,1);
end
for n=2:min(N,ceil(band*N)+1)
    D(1,n)=d(1,n)+D(1,n-1);
end
for m=2:M
    cn=1+round((N-1)/(M-1)*(m-1));
    for n=max(2,cn-ceil(band*N)):min(N,cn+ceil(band*N))
...
%     clc
    ...
    imagesc(D);
    cmap = contrast(D);
    colormap([jet;[0,0,0]]); % 'copper' 'bone', 'gray' imagesc(D);
    hold on;
    x=w(:,1); y=w(:,2);
    ...
    % --- Warped signals
    figure('Name','DTW - warped signals', 'NumberTitle','off','units','normalized','outerposition',[0 .1 1 .9]);
    
    subplot(2,2,1:2);
    set(gca, 'FontSize',7);
    hold on;
    plot(r,'-bx','LineWidth',3,'MarkerSize',10);
    plot(t,'-r*','LineWidth',3,'MarkerSize',10);
    hold off;
    ...
    set(gca, 'FontSize',7);
    hold on;
    plot3(1:length(r),r,zeros(1,length(r)),'-bx','LineWidth',3,'MarkerSize',10);
    plot3(1:length(t),t,.25*k*ones(1,length(t)),'-r*','LineWidth',3,'MarkerSize',10);
    for i=1:k
        plot3([w(i,1),w(i,2)],[rw(i),tw(i)],[0,.25*k],'k','LineWidth',2);
    end
    hold off;
    axis tight;
    pbaspect([2,1,0.5]);
...
%     axis([1 max(M,N) min(min(r),min(t)) 1.1*max(max(r),max(t))]);
    ...
    set(gca, 'FontSize',7);
    hold on;
    plot(rw,'-bx','LineWidth',3,'MarkerSize',10);
    plot(tw,'-r*','LineWidth',3,'MarkerSize',10);
    hold off;
